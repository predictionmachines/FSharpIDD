...
{
 "use strict";
 var Global,FSharpIDD,Html,Attribute,Div,Colour,Colour$1,Plot,Polyline,Plot$1,WebSharper,Obj,Options,Chart,Chart$1,SC$1,List,Strings,Arrays,Seq,IntelliFactory,Runtime,Utils,Char,ArgumentException;
 Global=self;
 FSharpIDD=Global.FSharpIDD=Global.FSharpIDD||{};
 Html=FSharpIDD.Html=FSharpIDD.Html||{};
 Attribute=Html.Attribute=Html.Attribute||{};
 Div=Html.Div=Html.Div||{};
 Colour=FSharpIDD.Colour=FSharpIDD.Colour||{};
 Colour$1=Colour.Colour=Colour.Colour||{};
 Plot=FSharpIDD.Plot=FSharpIDD.Plot||{};
 Polyline=Plot.Polyline=Plot.Polyline||{};
 Plot$1=Polyline.Plot=Polyline.Plot||{};
 WebSharper=Global.WebSharper;
 Obj=WebSharper&&WebSharper.Obj;
 Options=Polyline.Options=Polyline.Options||{};
 Chart=FSharpIDD.Chart=FSharpIDD.Chart||{};
 Chart$1=Chart.Chart=Chart.Chart||{};
 SC$1=Global.StartupCode$FSharpIDD$FSharpIDD=Global.StartupCode$FSharpIDD$FSharpIDD||{};
 ...
 ArgumentException=WebSharper&&WebSharper.ArgumentException;
 Attribute.New=function(Key,Value)
 {
  return{
   Key:Key,
   Value:Value
 ...
 Div.New=function(Attributes,Children)
 {
  return{
 ...
 Html.divToStr=function(div)
 {
  var attributesStr,contents;
  attributesStr=List.length(div.Attributes)===0?"":" "+Strings.Join(" ",Arrays.ofSeq(Seq.map(function(attr)
  {
   return(((Runtime.Curried3(function($1,$2,$3)
   {
    return $1(Utils.toSafe($2)+"=\""+Utils.toSafe($3)+"\"");
   }))(Global.id))(attr.Key.$0))(attr.Value.$0);
  },div.Attributes)));
  contents=Strings.Join("\n",Arrays.ofSeq(Seq.map(function(node)
  {
   return node.$==1?Html.divToStr(node.$0):node.$0.$0;
  },div.Children)));
  return(((Runtime.Curried3(function($1,$2,$3)
  {
   return $1("<div"+Utils.toSafe($2)+">"+Utils.toSafe($3)+"</div>");
  }))(Global.id))(attributesStr))(contents);
 };
 Html.addAttribute=function(key,value,div)
 {
  return Div.New(new List.T({
   $:1,
   $0:Attribute.New(Html.guardAttrName(key),Html.guardText(value)),
   $1:div.Attributes
  }),div.Children);
 };
 Html.addDiv=function(child,parent)
 {
  return Div.New(parent.Attributes,new List.T({
   $:1,
   $0:{
    $:1,
    $0:child
 ...
 Html.addText=function(text,div)
 {
  return Div.New(div.Attributes,new List.T({
   $:1,
   $0:{
    $:0,
    $0:Html.guardText(text)
   },
   $1:div.Children
  }));
 };
 ...
 {
  return Div.New(List.T.Empty,List.T.Empty);
 };
 Html.guardText=function(str)
 {
  var validChars;
  validChars=["-","[","]","(",")",":",";",".",","];
  if(Arrays.exists(function(c)
  {
   return!(Char.IsWhiteSpace(c)||Char.IsLetterOrDigit(c)||Arrays.contains(c,validChars));
  },Strings.ToCharArray(str)))
   throw new ArgumentException.New$2((((Runtime.Curried3(function($1,$2,$3)
   {
    return $1("the string must contain only letters, digits or "+Utils.printArray(Utils.prettyPrint,$2)+", but "+Utils.toSafe($3)+" was passed");
   }))(Global.id))(validChars))(str));
  else
   null;
  return{
   $:0,
 ...
 Html.guardAttrName=function(str)
 {
  var validChars;
  validChars=["-"];
  if(Arrays.exists(function(c)
  {
   return!(Char.IsLetterOrDigit(c)||Arrays.contains(c,validChars));
  },Strings.ToCharArray(str)))
   throw new ArgumentException.New$2((((Runtime.Curried3(function($1,$2,$3)
   {
    return $1("the string must contain only letters, digits or "+Utils.printArray(Utils.prettyPrint,$2)+", but "+Utils.toSafe($3)+" was passed");
 ...
 Colour.createColour=function(red,green,blue)
 {
  return Colour$1.New(red,green,blue);
 };
 Plot$1.New=function(Name,X,Y,Colour$2,Thickness)
 {
  return{
   Name:Name,
   X:X,
 ...
 Options=Polyline.Options=Runtime.Class({
  get_SpecifiedThickness:function()
  {
   return this.thickness;
  },
  set_Thickness:function(v)
  {
   this.thickness={
    $:1,
    $0:v
  ...
  get_SpecifiedColour:function()
  {
   return this.colour;
  },
  set_Colour:function(v)
  {
   this.colour={
    $:1,
    $0:v
 ...
 Options.New=Runtime.Ctor(function()
 {
  Obj.New.call(this);
  this.name=null;
  this.colour=null;
 ...
 Polyline.setThickness=function(thickness,polyline)
 {
  return Plot$1.New(polyline.Name,polyline.X,polyline.Y,polyline.Colour,{
   $:1,
   $0:thickness
  });
 };
 Polyline.setStrokeColour=function(colour,polyline)
 {
  return Plot$1.New(polyline.Name,polyline.X,polyline.Y,{
   $:1,
   $0:colour
  },polyline.Thickness);
 };
 Polyline.setName=function(name,polyline)
 {
  return Plot$1.New({
   $:1,
   $0:name
  },polyline.X,polyline.Y,polyline.Colour,polyline.Thickness);
 };
 Polyline.createPolyline=function(x,y)
 {
  return Plot$1.New(null,x,y,null,null);
 };
 Polyline.setOptions=function(options,polyline)
 {
  var polyline$1,m,polyline$2,m$1,m$2;
  polyline$1=(m=options.get_SpecifiedName(),m!=null&&m.$==1?Plot$1.New({
   $:1,
   $0:m.$0
  },polyline.X,polyline.Y,polyline.Colour,polyline.Thickness):polyline);
  polyline$2=(m$1=options.get_SpecifiedColour(),m$1!=null&&m$1.$==1?Plot$1.New(polyline$1.Name,polyline$1.X,polyline$1.Y,{
   $:1,
   $0:m$1.$0
  },polyline$1.Thickness):polyline$1);
  m$2=options.get_SpecifiedThickness();
  return m$2!=null&&m$2.$==1?Plot$1.New(polyline$2.Name,polyline$2.X,polyline$2.Y,polyline$2.Colour,{
   $:1,
   $0:m$2.$0
  }):polyline$2;
 };
 Chart$1.New=function(Size,Title,Xlabel,Ylabel,Plots)
 {
  return{
   Size:Size,
   Title:Title,
 ...
 Chart.toHTML=function(chart)
 {
  function polylineToDiv(p)
  {
   var styleEntries,styleEntries$1,m,resultNode,x,m$1,c,m$2;
   styleEntries=List.T.Empty;
   styleEntries$1=(m=p.Thickness,m==null?styleEntries:new List.T({
    $:1,
    $0:(function($1)
    {
     return function($2)
     {
      return $1("thickness: "+$2.toFixed(1));
     };
    }(Global.id))(m.$0),
    $1:styleEntries
   }));
   resultNode=(x=Html.addAttribute("data-idd-style",Strings.Join("; ",Arrays.ofSeq((m$1=p.Colour,m$1==null?styleEntries$1:(c=m$1.$0,new List.T({
    $:1,
    $0:((((Runtime.Curried(function($1,$2,$3,$4)
    {
     return $1("stroke: rgb("+Global.String($2)+","+Global.String($3)+","+Global.String($4)+")");
    },4))(Global.id))(c.R))(c.G))(c.B),
    $1:styleEntries$1
   }))))),Html.addAttribute("data-idd-plot","polyline",Html.createDiv())),Html.addText(Seq.fold2(function($1,$2,$3)
   {
    return $1+(((Runtime.Curried3(function($4,$5,$6)
    {
     return $4("\u0009"+$5.toFixed(6)+"\u0009"+$6.toFixed(6)+"\n");
    }))(Global.id))($2))($3);
   },"\u0009x\u0009y\n",p.X,p.Y),x));
   m$2=p.Name;
   return m$2==null?resultNode:Html.addAttribute("data-idd-name",m$2.$0,resultNode);
  }
  return Html.divToStr(Seq.fold(function($1,$2)
  {
   return Html.addDiv($2,$1);
  },Html.addAttribute("style","width: 800px; height: 600px;",Html.addAttribute("data-idd-plot","chart",Html.addAttribute("class","fsharp-idd",Html.createDiv()))),Seq.map(function(plot)
  {
   return polylineToDiv(plot.$0);
  },chart.Plots)));
 };
 Chart.addPolyline=function(polyline,chart)
 {
  return Chart$1.New(chart.Size,chart.Title,chart.Xlabel,chart.Ylabel,new List.T({
   $:1,
   $0:{
    $:0,
    $0:polyline
 ...
 {
  SC$1.$cctor=Global.ignore;
  SC$1.Red=Colour.createColour(255%256,0%256,0%256);
  SC$1.Green=Colour.createColour(0%256,255%256,0%256);
  SC$1.Blue=Colour.createColour(0%256,0%256,255%256);
  SC$1.Empty=Chart$1.New(null,null,null,null,List.T.Empty);
 };
}());
...
{
 "use strict";
 var Global,Demo,WebSharper,Arrays,Math,FSharpIDD,Plot,Polyline,Options,Colour,Chart,Plot$1;
 Global=self;
 Demo=Global.Demo=Global.Demo||{};
 WebSharper=Global.WebSharper;
 Arrays=WebSharper&&WebSharper.Arrays;
 Math=Global.Math;
 FSharpIDD=Global.FSharpIDD;
 Plot=FSharpIDD&&FSharpIDD.Plot;
 Polyline=Plot&&Plot.Polyline;
 Options=Polyline&&Polyline.Options;
 Colour=FSharpIDD&&FSharpIDD.Colour;
 Chart=FSharpIDD&&FSharpIDD.Chart;
 Plot$1=Polyline&&Polyline.Plot;
 Demo.getChart=function()
 {
  var Xseries,Yseries1,Yseries2,Yseries3,curve1,r,curve2,curve3;
  Xseries=Arrays.init(1000,function(i)
  {
   return(i+1)*0.01;
  });
  Yseries1=Arrays.map(function(x)
  {
   return Math.sin(2*x)/x;
  },Xseries);
  Yseries2=Arrays.map(function(x)
  {
   return Math.sin(5*x)/x;
  },Xseries);
  Yseries3=Arrays.map(function(x)
  {
   return Math.sin(5*x+1)/x;
  },Xseries);
  curve1=Polyline.setOptions((r=new Options.New(),r.set_Name("Curve 1"),r),Polyline.createPolyline(Xseries,Yseries1));
  curve2=Polyline.setThickness(2,Polyline.setStrokeColour(Colour.Green(),Polyline.setName("Curve 2",Polyline.createPolyline(Xseries,Yseries2))));
  curve3=Polyline.createPolyline(Xseries,Yseries3);
  return Chart.toHTML(Chart.addPolyline(Plot$1.New({
   $:1,
   $0:"Curve 3"
  },curve3.X,curve3.Y,curve3.Colour,{
   $:1,
   $0:1
  }),Chart.addPolyline(curve2,Chart.addPolyline(curve1,Chart.Empty()))));
 };
}());
