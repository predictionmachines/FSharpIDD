module FSharpIDD.HtmlConverters
...
let axisToHtmlStructure axis placement parent =
    let getDataDomWithTicksLabels ticks labels =
            ...
            str        
    match axis with
        |   Axis.Hidden -> parent, Option.None
        |   Axis.Numeric(scientificNotationEnabled) ->
            ...
            (parent |> addDiv axisNode),(Some id)
...
let toHtmlStructure (chart:Chart) =
    ...
    let chartNode,yAxisID = axisToHtmlStructure chart.Yaxis Left chartNode
    ...
    let chartNode,xAxisID = axisToHtmlStructure chart.Xaxis Bottom chartNode

    let chartNode = 
        if chart.Xlabel <> null then
            horAxisLabelToHtmlStructure chart.Xlabel Left chartNode
        else
            chartNode
